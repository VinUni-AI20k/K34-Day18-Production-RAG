...
def evaluate_ragas(questions: list[str], answers: list[str],
    ...
    try:
        ...
        # LLM chạy qua provider OpenAI-compatible (OpenRouter nếu OPENAI_BASE_URL set).
        # Embeddings dùng model local để không phụ thuộc API.
        ...
        return {
            "faithfulness": float(result["faithfulness"] or 0.0),
            "answer_relevancy": float(result["answer_relevancy"] or 0.0),
            "context_precision": float(result["context_precision"] or 0.0),
            "context_recall": float(result["context_recall"] or 0.0),
